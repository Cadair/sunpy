...
def test_wcs(aia171_test_map):
    ...
    assert all(wcs.wcs.cdelt == [aia171_test_map.scale.x.value, aia171_test_map.scale.y.value])
    assert all(wcs.wcs.crval == [
        aia171_test_map.reference_coordinate.x.value, aia171_test_map.reference_coordinate.y.value
    ])
    assert set(wcs.wcs.ctype) == set(
        [aia171_test_map.coordinate_system.x, aia171_test_map.coordinate_system.y])
...
def test_data_range(generic_map):
    ...
    assert generic_map.xrange[1].value - generic_map.xrange[0].value == generic_map.meta[
        'cdelt1'] * generic_map.meta['naxis1']
    assert generic_map.yrange[1].value - generic_map.yrange[0].value == generic_map.meta[
        'cdelt2'] * generic_map.meta['naxis2']
...
def test_shift_applied(generic_map):
    ...
    original_reference_coord = (generic_map.reference_coordinate.x,
                                generic_map.reference_coordinate.y)
    x_shift = 5 * u.arcsec
    y_shift = 13 * u.arcsec
    ...
    crval1 = (
        (generic_map.meta.get('crval1') * generic_map.spatial_units.x + shifted_map.shifted_value.x
         ).to(shifted_map.spatial_units.x)).value
    assert shifted_map.meta.get('crval1') == crval1
    crval2 = (
        (generic_map.meta.get('crval2') * generic_map.spatial_units.y + shifted_map.shifted_value.y
         ).to(shifted_map.spatial_units.y)).value
